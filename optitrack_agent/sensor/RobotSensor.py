...
class RobotSensor():
    def __init__(self, rigidBodyDict={1:Rigidbody(1), 2:Rigidbody(2), 3:Rigidbody(3)}):
        ...
        # calibrate prepare
        self.calibrate_list = [False, False, False]
        self.dist = [0,0,0]
        self.ang = [0,0,0]
...
if __name__ == "__main__" and True:
    ...
    exp_name = "静力学控制实验"
    time_rela_list = []     # 相对时间
    time_abs_list = []      # 系统时间，作为绝对时间
    distance_list_1 = []    # 读取的距离值1
    distance_list_2 = []    # 读取的距离值2
    distance_list_3 = []    # 读取的距离值3
    theta_list_1 = []       # 读取的角度值1
    theta_list_2 = []       # 读取的角度值2
    theta_list_3 = []       # 读取的角度值3
    ...
    while run_flag:
        try:
            ...
            distance_list_1.append(sensor.dist[0])
            distance_list_2.append(sensor.dist[1])
            distance_list_3.append(sensor.dist[2])
            theta_list_1.append(sensor.theta[0])
            theta_list_2.append(sensor.theta[1])
            theta_list_3.append(sensor.theta[2])
            time.sleep(0.2)
            ...
            with open(file_name, "w", newline="") as f:
                ...
                csv_writer.writerow(["Time", "Time_abs", "Distance1", "Distance2", "Distance3", "Theta1", "Theta2", "Theta3"])
                for i in range(len(time_rela_list)):
                    csv_writer.writerow([time_rela_list[i], time_abs_list[i], distance_list_1[i], distance_list_2[i], distance_list_3[i], theta_list_1[i], theta_list_2[i], theta_list_3[i]])
